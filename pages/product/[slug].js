          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {Object.keys(variants).includes('white') && Object.keys(variants['white']).includes(size) && <button onClick={(e) => { refreshVariant(size, 'white') }} className={`border-2  bg-white rounded-full w-6 h-6 focus:outline-none mx-1 ${color === 'white' ? 'border-black' : 'border-gray-300'}`}></button>}
                {Object.keys(variants).includes('red') && Object.keys(variants['red']).includes(size) && <button onClick={(e) => { refreshVariant(size, 'red') }} className={`border-2  bg-red-700 rounded-full w-6 h-6 focus:outline-none mx-1 ${color === 'red' ? 'border-black' : 'border-gray-300'}`}></button>}
                {Object.keys(variants).includes('green') && Object.keys(variants['green']).includes(size) && <button onClick={(e) => { refreshVariant(size, 'green') }} className={`border-2  bg-green-700 rounded-full w-6 h-6 focus:outline-none mx-1 ${color === 'green' ? 'border-black' : 'border-gray-300'}`}></button>}
                {Object.keys(variants).includes('blue') && Object.keys(variants['blue']).includes(size) && <button onClick={(e) => { refreshVariant(size, 'blue') }} className={`border-2  bg-blue-700 rounded-full w-6 h-6 focus:outline-none mx-1 ${color === 'blue' ? 'border-black' : 'border-gray-300'}`}></button>}
                {Object.keys(variants).includes('yellow') && Object.keys(variants['yellow']).includes(size) && <button onClick={(e) => { refreshVariant(size, 'yellow') }} className={`border-2 bg-yellow-700 rounded-full w-6 h-6 focus:outline-none mx-1 ${color === 'yellow' ? 'border-black' : 'border-gray-300'}`}></button>}
              </div>
            ...
            <form>
              <div className="pin mt-6 flex space-x-2 text-sm">
                <input type="text" name="pincode" id="pincode" className='px-2 border-2 border-gray-400 rounded-lg' placeholder='Enter your Pincode' onChange={(e) => { setPincode({ ...pincode, [e.target.name]: e.target.value }) }} />
                <button className="flex ml-auto md:ml-14 text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded" onClick={checkServiceability}>Check</button>
              </div>
            </form>
            {delivery}
          </div>
...
export async function getServerSideProps(context) {
  ...
  return {
    props: { product: JSON.stringify(product), variants: JSON.stringify(colorSizeSlug) }
  }
}
